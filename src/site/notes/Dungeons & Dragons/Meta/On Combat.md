...
- The structure of requiring multiple player-to-DM pings per turn carries a fundamental hinderance in maintaining fluidity in the transition between the loose, exploratory nature of D&D and the rules-strict, high-stakes wargames that are combat encounters.
	- **Current structure:**
		...
		- DM: You hit. Check for the next outcome.
		- Player: I check for the next outcome.
		- DM: This is the next outcome.
	...
	- It often seems like the best combat encounters are those that are indistinguishable from the rest of the game, or even where actual, direct *fighting* is not happening. 
		...
		- Another example, much more common than the first, is when the players are in a *transition between combat*. Think stealth through a bandit camp, where you're whispering across the table (or mic) as your characters tip-toe around sleeping enemies—but being seen immediately demands the initiation of the combat ruleset, thus inserting an uncanny *break* while everyone rolls initiative and the DM scrambles to do the same (on top of their regular duties).
			- The initial call for initiative can often be tense, but it is nonetheless a break in the flow of the game. Whatever the preestablished momentum or *vibe*, it is reset in favor of the new 'game' that we swap to.
## On Fluff & Flavor
...
- Highly effective as a template for an interesting encounter, but fails to address the underlying issues in the 5e combat cycle, and limitations that some DMs face.
## DM's Limitations
- Improvisation and multitasking are both difficult for me as it relates to playing D&D (as player and DM). It's an issue that seems to persist through multiple layers of preparation—you might notice it most prominently in forgetting NPCs and events (within combat and without), changing voices or personalities, and tracking combat in general.
...
- You no longer make attack rolls. Instead, roll damage directly.
	...
	- If you make your highest roll (4 on a d4), you crit and roll that die again, adding it to the result.
		- Effects that alter your crit chance still apply (i.e. 3-4 being a crit on a d4).
	- For damage rolls that use multiple dice (i.e. Greatsword's 2d6), there are three methods available to resolve the attack:
		1. The player must, prior to the roll, select one die in the array to be the *Primary Die*: This die will determine a hit, miss, or crit. This method is more balanced.
		2. Following the first method, your lowest roll becomes your *Primary Die.*
		3. Each die is its own 'mini' attack roll: Missing or critting with one has no impact on the other. This is a more chaotic method, ~~and it's objectively more fun and immersive than the former methods~~.
...
- **Con:**
	...
	- Yet another house rule to remember.
# Conclusions
---
...
- **Stakes.** Combat encounters should never exist just because you need combat encounters. Units on the field exist as obstacles, the same as a risky bridge, and the players should have more reasons for navigating them than doing so for the hell of it. Do *really* only just need to cross the bridge? Then the grass on the other side better be pretty damn green.
- **Dilemma.** The stage is set and the mission is clear—but what's stopping the players from reaching their goal? You'd better hope it's not *yet another attack roll*. If it's come down to a series of monotone attack rolls on either side, you should have made them do it while it was raining fire and the floor was lava on every odd turn.
- **Engagement.** To what degree can the players interact with their surroundings? The more the better. If they're forced to as a mechanism of the encounter? Perfect—just give them a warning round if they don't get it at first.
- **Immersion.** The most nebulous concept here, but it probably lies downstream from these other components, in combination (in small part) with the group's preestablished acting and storytelling experience and application thereof.
## From Here?
